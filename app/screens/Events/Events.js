...
export default function Events({navigation}) {
  ...
  useEffect(() => {
    AsyncStorage.getItem('whsnxt_user_data')
      .then(res => {
        console.log(res);
        if (res) {
  ...
  const eventRender = data => {
    ...
    data?.map((item, index) => {
      ...
      const date = edata
        ? moment(edata).format('DD MMM [@]YYYY')
        : '17 Dec,@2022';
      ...
      row.push(
        <View
          ...
          onTouchEnd={() => navigation.navigate('EventDetail', {id: eventId})}>
          <ImageBackground
            source={require('./back.png')}
            style={{width: '100%', aspectRatio: 302 / 122, height: 'auto'}}>
            <Text
              style={{
                fontSize: 14,
                fontWeight: '600',
              ...
              {name ? name : 'National Seminar'}
            </Text>
            <Text style={{fontSize: 12, color: 'white', marginLeft: 20}}>
              {artist ? artist : 'By Jobh Doe'}
            </Text>
            <View
              style={{
              ...
              <Text style={{color: 'white', fontSize: 12, alignSelf: 'center'}}>
                {date !== 'Invalid date' ? date : '17 Dec,@2022'}
              </Text>
            </View>
            <Image
              source={require('./i_star.png')}
          ...
          <View style={{flexDirection: 'row', padding: 20}}>
            ...
            <Text
              ...
              Whitefeeld,Lahore, 642002,Pakistan
            </Text>
            <View
              style={{
                flexDirection: 'column',
              ...
              <Text style={{fontSize: 9, color: 'black'}}>
                {distance ? Math.round(distance) : '0'} km from here
              </Text>
              <Text style={{fontSize: 9, color: 'black'}}>
                {time !== 'Invalid date' ? time : '00:00 AM'}
              </Text>
            </View>
          </View>
          <View style={{flexDirection: 'row'}}>
            <Image
              source={require('./i_users.png')}
    ...
    <SafeAreaView style={styles.PrivacyPolicy}>
      ...
      {filterMenuVisible ? (
        <View
          ...
          <View style={{height: 40, borderBottomWidth: 1, width: '100%'}}>
            <Text
              style={{marginTop: 10, fontSize: 12, alignSelf: 'center'}}
